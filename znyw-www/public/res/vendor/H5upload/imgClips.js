...
// constructor----------------------------------------
var ImgClips = function(opt){
	this.canvas = null; // canvas对象
	this.fileObj = null; // file对象
	this.cutBtn = null; // 触发剪切的按钮对象
	this.resultObj = null; // 输出图片对象
	this.upload = null; // 上传对象
	this.againObj = null; // againObj对象
	this.imgData = null; // againObj对象
...
ImgClips.prototype = {
	init: function(opt) { // 初始化
		...
		this.againObj = this.getObj(opt.againObj);
		// this.cutBtn = this.getObj(opt.cutBtn);
		this.upload = this.getObj(opt.upload);
		this.resultObj = this.getObj(opt.resultObj);
        this.imgData = this.getObj(opt.imgData);
		// save data
		...
		addEvent(this.fileObj, 'change', function(e){
            $(".H5upload2").show();
           	...
           	// document.getElementsByClassName('H5upload').style.display="block";
        ...
        // 重新上传图片
        // addEvent(this.againObj, 'change', function(e){
        //     // document.getElementsByClassName('H5upload').style.display="block";
        //     if(!e.target.files[0]) return;
        //     This.r = 1;
        //     This.rot = 0;
        ...
        //     This.run(e,This);
        // });
        addEvent(this.againObj, 'click', function(e){
            // document.getElementsByClassName('H5upload').style.display="block";
            $(".H5upload2").hide();
            return
        });
		...
		// });
		// 上传
        addEvent(this.upload, 'click', function(e){
            This.getResultsUpload(e);
            $(".H5upload2").hide();
        });
	...
	getResults: function () { // 裁剪结果输出
        ...
        this.bs64URL=canvas2.toDataURL();


    },
    getResultsUpload: function () { // 裁剪结果输出
        if(!this.dataURL) return;
...
//                        url: "http://bld.dianlibian.com/api/web_common/uploadMultiImage",
            ...
            success: function (datas) {
                imgdata.imgData.value = datas.data[0].url;
            },
            error: function () { //上传失败
                console.log("error");
                alert("上传失败请重新上传;");
