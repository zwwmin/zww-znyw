 ...
 */
var AjaxFileClick=function(opts){
    var settings={
        ...
        ,max_size:2*1024*1024 //限制上传大小--注意，在ie6-9上面是没办法在客户端获取尺寸的，所以这种时候这个属性相当于没用。
        ,maxError:function(){} //超过上传大小后执行
        ,url:"" //上传时候的url。
    ...
    var _clickBtn=$(settings.clickBtn);
    ...
    var _i_app={
        ...
        ,resetFileInput:function(__file_input){
            ...
            $(_clickBtn).click(function(){
                ...
                if(_fileDom.files!=undefined){
                    if( _fileDom.files[0].size>settings.max_size ){
                        console.log('超过上传大小限制');
                        settings.maxError(settings.max_size);
                        return;
                    }
                }
